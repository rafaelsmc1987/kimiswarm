# Yahoo Finance Kimi Plugin

This plugin bundles a Kimi skill and helper script for querying the Yahoo Finance
datasource through `agent-gw`.

## Contents

- `kimi.plugin.json`: catalog manifest for the Kimi plugin.
- `skills/yahoo_finance/SKILL.md`: model instructions for selecting and calling
  Yahoo Finance datasource APIs.
- `scripts/yahoo_finance_tool.py`: command-line wrapper around
...
python3 scripts/yahoo_finance_tool.py call --api-name "<api name>" --params-json '{"ticker":"AAPL"}'
```
