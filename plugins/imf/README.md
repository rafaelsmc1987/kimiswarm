...
This plugin bundles a Kimi skill and helper script for querying the IMF
datasource through `agent-gw`.

## Contents

- `kimi.plugin.json`: catalog manifest for the Kimi plugin.
- `skills/imf/SKILL.md`: model instructions for selecting and calling IMF
  datasource APIs.
- `scripts/imf_tool.py`: command-line wrapper around `get_data_source_desc`
  and `call_data_source_tool`.
...
python3 scripts/imf_tool.py call --api-name "<api name>" --params-json '{"country":"United States"}'
```
