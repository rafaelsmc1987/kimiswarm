...
This plugin bundles a Kimi skill and helper script for querying the Scholar
datasource through `agent-gw`.

## Contents

- `kimi.plugin.json`: catalog manifest for the Kimi plugin.
- `skills/scholar/SKILL.md`: model instructions for selecting and calling
  Scholar datasource APIs.
- `scripts/scholar_tool.py`: command-line wrapper around
...
python3 scripts/scholar_tool.py call --api-name "<api name>" --params-json '{"query":"large language models"}'
```
