...
;// CONCATENATED MODULE: ./web/chromecom.js
      ...
      getEmbedderOrigin(function (origin) {
        if (origin && !/^file:|^chrome-extension:/.test(origin)) {
          viewerApp._documentError(null, {
            message: `Blocked ${origin} from loading ${file}. Refused to load ` + "a local file in a non-local page for security reasons."
          });
          return;
        }
...
class ExternalServices extends BaseExternalServices {
  initPassiveLoading() {
    ChromeCom.resolvePDFFile(AppOptions.get("defaultUrl"), function (url, length, originalUrl) {
      viewerApp.open({
        url,
        length,
        originalUrl
  ...
  async createL10n() {
    return new genericl10n_GenericL10n(navigator.language);
  }
  createScripting() {
...
class PDFHistory {
  ...
  #pushOrReplaceState(destination, forceReplace = false) {
    ...
    if (window.history.state?.chromecomState) {
  ...
  #popState({
    ...
    if (state?.chromecomState && !this.#isValidState(state) || !state) {
      this._uid++;
      const {
        hash,
        page,
...
let pdf_print_service_viewerApp = {
  initialized: false
};
...
function ensureOverlay() {
  if (!overlayPromise) {
    overlayManager = pdf_print_service_viewerApp.overlayManager;
    if (!overlayManager) {
...
class PDFPrintServiceFactory {
  static initGlobals(app) {
    pdf_print_service_viewerApp = app;
  }
...
const pdfjsVersion = "4.6.129";
const pdfjsBuild = "3af2ecd6d";
