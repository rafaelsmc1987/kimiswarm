...
def hook_task_created(task: TaskSpec) -> GateDecision:
    ...
    checks = [
        ...
        _check(
            "SCHEMA",
            "task has an output schema or acceptance criteria",
    ...
    return GateDecision.compose(
        f"hook:task_created:{task.task_id}", GateKind.PLAN, checks
    )
...
def hook_pre_tool_use(
    ...
    *,
    ...
    if authorized_tools is not None:
        ...
        checks.append(
            _check(
                "TOOL_AUTHORIZED", f"tool '{tool_name}' is authorized", name in allowed
            )
        )
    ...
    if name in _WRITE_TOOLS:
        ...
        if run_root and isinstance(target, str) and target:
            checks.append(
                _check(
                    "WRITE_IN_SCOPE",
                    "write target stays within the run root",
                    is_within(run_root, target),
                    target,
                )
            )
    ...
    if any(
        ...
        checks.append(
            _check(
                "NO_CMD_INJECTION",
                "command is not a known injection pattern",
                False,
                command,
            )
        )
    ...
    if name in _READ_TOOLS:
        needle = (
            str(tool_input.get("query", "")) + " " + str(tool_input.get("pattern", ""))
        )
        hits = scan_secrets(needle)
        checks.append(
            _check(
                "NO_SECRET_READ",
                "read does not target a secret",
                not hits,
                [h.redacted() for h in hits],
            )
        )
    ...
    return GateDecision.compose(
        f"hook:pre_tool_use:{tool_name}", GateKind.SECURITY, checks
    )
...
def hook_subagent_stop(result: AgentResult, task: TaskSpec) -> GateDecision:
    ...
    checks = [
        _check(
            "VALID_OUTPUT",
            "agent produced the declared outputs",
            result.covers_outputs(task.outputs),
        ),
        _check(
            "EVIDENCE_REFS",
            "agent supplied evidence refs",
        ...
        _check(
            "TESTS_RUN",
            "declared tests were actually executed",
            result.tests_actually_executed(),
        ),
    ]
    return GateDecision.compose(
        f"hook:subagent_stop:{task.task_id}", GateKind.CLAIM, checks
    )
...
def hook_task_completed(task: TaskSpec, result: AgentResult) -> GateDecision:
    ...
    for crit in task.acceptance.criteria:
        met = any(crit in out for out in result.outputs_produced) or crit in str(
            result.payload
        )
    ...
    return GateDecision.compose(
        f"hook:task_completed:{task.task_id}", GateKind.PLAN, checks
    )
...
def hook_stop(
    *,
    ...
    checks = [
        ...
        _check(
            "CRITICAL_RESOLVED",
            "critical claims resolved or disclosed",
            not unresolved_critical,
        ),
        _check("INTEGRITY_PASS", "final integrity gate passed", integrity_pass),
        _check(
            "DELIVERY_MANIFEST", "delivery manifest present", bool(delivery.artifacts)
        ),
