...
Determine the output scenario and canvas first, then design the grid, font sizes, and content structure. Do not equate a poster with portrait orientation by default, and do not default to 16:9.
...
5. When the user does not specify a ratio, use adaptive height by default and determine the canvas height from the content and output scenario.

| Common ratio | PPTD reference size |
|---|---:|
| 16:9 | `[1280, 720]` |
| 9:16 | `[720, 1280]` |
| 4:3 | `[1280, 960]` |
...
- Grids, whitespace, cards, collages, asymmetry, or full-bleed are all optional means, not the default correct answer. Avoid stuffing every category of content below the title into equal-width, equal-height rectangles with rounded corners, outlines, or shadows, forming a "three-card/four-card + CTA" content-page or app-interface skeleton. Use containers only when the container itself has clear semantics and will not form a uniform card wall; otherwise prefer organizing content with layout zones, proportion, color blocks, rule lines, spatial adjacency, object groups, or typographic rhythm.
