...
PPTD is a YAML-format design DSL for describing, generating, and editing design deliverables (presentations, posters, infographics, etc.) in an AI-friendly way; it supports lossless bidirectional conversion with PPTX and can also be exported as images
...
- Dimensions use px; at 72 PPI, 1px = 1pt. The origin `(0, 0)` is the top-left corner of the page
...
For conflicting property values, search the following sources in order from top to bottom; when none of the levels is set, fall back to the default values.
...
1. A series' explicit `fill` / `lineColor` / `areaColor` (field names vary by type; see the corresponding series definitions)
2. The same-named field for the corresponding type in [Chart.seriesDefaults](#seriesdefaults)
3. The renderer's built-in 10-color chart palette (colors are picked in the order the corresponding type's series, slices, or nodes appear, then cycle from the beginning after 10 colors)

> The built-in palette, in order, is: `#1783FE`, `#67B8A6`, `#C9A869`, `#8A84C7`, `#8C98A8`, `#5E9CF4`, `#7AB5D7`, `#B08A9C`, `#333333`, `#DCDCDC`.
>
> [scatter](#scatter) is an exception: marker color resolves as `marker.fill > series.fill > built-in 10-color chart palette`; marker.border likewise takes precedence over series.border.
...
3. Theme default ([Theme](#theme)); falls back to the global default font `MiSans` when unset
...
> Sub-fields follow a **one-level shallow merge**: `series.dataLabels` only overrides the sub-fields it explicitly provides; unprovided ones fall back from [Chart.dataLabels](#chart); if neither provides them, use the default defined by the corresponding series.
...
4. **Only Google Fonts are supported as custom fonts**: `customFonts[].src` must use a URL in the format `https://fonts.googleapis.com/css2?family=Noto+Serif+SC`
...
interface Page {
  ...
  animations?: Animation[];        // default: none; orchestrated in array order
}
```
...
> **Limitation:** tables cannot be rotated/flipped as a whole; whole-table global opacity including text and borders is also not supported.
...
interface Chart extends ElementBase {
  ...
  // —— Cartesian coordinate system (conditionally effective by series.type; see "Applicability of Chart Top-Level Fields" below) ——
  ...
  fontFamily?: FontFamily;                    // default: falls back along the theme fonts
...
>
> **Limitation:** charts cannot be rotated/flipped as a whole; there is also no single global opacity property covering "the whole chart including title, axes, legend, labels, and series".
...
3. **Conditionally effective top-level fields**: `xAxis` / `yAxis` / `barWidth` / `barGap` / `categoryGap` / `spokeAxis` are **coordinate-system-level** configurations, conditionally effective based on the `series[].type` set (see [Applicability of Chart Top-Level Fields](#applicability-of-chart-top-level-fields) for details).
...
> **encode naming convention**: Cartesian coordinate systems use `x` / `y`; non-Cartesian coordinate systems use `category` for category fields; sankey uses `source` / `target` for graph-edge endpoints.
>
...
```ts
interface TextStyle {
  color?: Color;            // default: falls back along the inheritance chain (theme text color, ultimately #000000)
...
interface DataLabelConfig extends TextStyle {
  ...
  content?: "value" | "percentage" | "category";  // default: varies by type; see the dataLabels comment of the corresponding series
  numberFormat?: string;               // default: no formatting; Excel number-format string (see below)
}
```

> For each type, see the corresponding series definition for the allowed `content` values and defaults; when a type is given an unsupported value, the checker raises `InvalidValueError`.
>
...
interface LinearSeriesBase {
  ...
  lineColor?: Color | GradientFill;                   // default: cycles through the built-in 10-color chart palette; line color of line / polygon stroke color of area+radar
}
```
...
interface BarSeries {
  ...
  fill?: Color | GradientFill;         // default: cycles through the built-in 10-color chart palette
  border?: Border;                     // default: not applied
...
interface ScatterSeries {
  ...
  fill?: Color | GradientFill;         // default: cycles through the built-in 10-color chart palette; serves as the marker's default fill color (marker.fill takes precedence)
  border?: Border;                     // default: not applied; serves as the marker's default border (marker.border takes precedence)
  dataLabels?: DataLabelConfig;        // default: not shown; content only takes "value"
}
```

> Multiple scatter series may use a wide table with missing cells padded by `null`, with each series referencing its own `x` / `y` columns; alternatively, use `dataFilter` to group a long table.
#### bubble
...
interface BubbleSeries {
  ...
  fill?: Color | GradientFill;         // default: cycles through the built-in 10-color chart palette; bubble fill color
  border?: Border;                     // default: not applied
  dataLabels?: DataLabelConfig;        // default: not shown; content only takes "value"
}
```
...
interface PieSeries {
  ...
  fill?: Color | GradientFill | (Color | GradientFill)[];   // default: cycles by slice through the built-in 10-color chart palette; an array cycles by slice
  border?: Border;                     // default: not applied
...
interface TreemapSeries {
  ...
  fill?: Color | GradientFill
       | (Color | GradientFill)[]
       | (Color | GradientFill)[][];   // default: cycles through the built-in 10-color chart palette; see "color derivation rules" below
  border?: Border;                     // default: not applied
...
interface SunburstSeries {
  ...
  levels?: number;                     // default: show all levels
  fill?: Color | GradientFill | (Color | GradientFill)[];   // default: cycles by top-level node through the built-in 10-color chart palette; an array cycles by top-level node
  border?: Border;                     // default: not applied
  dataLabels?: DataLabelConfig;        // default: not shown; content takes "value" | "category", default "category"
}
```
...
interface SankeySeries {
  ...
  fill?: Color | GradientFill                     // default: cycles through the built-in 10-color chart palette (colors picked in node topological order)
       | (Color | GradientFill)[]                // an array cycles by node
       | Record<string, Color | GradientFill>;   // mapped by node name; unspecified nodes fall back to the built-in 10-color chart palette
  border?: Border;                     // default: not applied
  dataLabels?: DataLabelConfig;        // default: not shown; content takes "value" | "category", default "value"
}
```
...
#### Cross-Type Quick Reference

##### Applicability of Chart Top-Level Fields
...
##### Type-Mixing Compatibility Quick Reference
